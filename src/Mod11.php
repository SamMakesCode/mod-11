...
{
    /**
     * Calculates a checksum using the Mod11 methodology
     * @param string $data
     * @param array $weights
     * @param int $when_11
     * @param int $when_10
     * @return int
     */
    public static function calculate($data, $weights, $when_11 = 5, $when_10 = 0) : int
    {
        // If data is not a string...
        if (!is_string($data)) {
            throw new \InvalidArgumentException('data argument must be of type string.');
        }

        // If the weights is not an array
        if (!is_array($weights)) {
            throw new \InvalidArgumentException('weights argument must be of type array.');
        }
